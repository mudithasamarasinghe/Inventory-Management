...
Class Roles extends DBConnection {
	...
	public function save_roles(){
		...
		if(isset($oldpassword)){
			if(md5($oldpassword) != $this->settings->roledata('password')){
				return 4;
			}
		}
		$chk = $this->conn->query("SELECT * FROM `role` where role ='{$username}' ".($id>0? " and roleID!= '{$id}' " : ""))->num_rows;
		if($chk > 0){
			return 3;
			exit;
		}
		...
		if(empty($id)){
			$qry = $this->conn->query("INSERT INTO role set {$data}");
			if($qry){
				$id = $this->conn->insert_id;
				$this->settings->set_flashdata('success','User role Details successfully saved.');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
			...
			$qry = $this->conn->query("UPDATE role set $data where roleID = {$id}");
			if($qry){
				$this->settings->set_flashdata('success','User Role Details successfully updated.');
				if($id == $this->settings->roledata('id')){
					foreach($_POST as $k => $v){
						if($k != 'id'){
							if(!empty($data)) $data .=" , ";
							$this->settings->set_roledata($k,$v);
						}
					}
...
switch ($action) {
	case 'save':
		echo $users->save_roles();
	break;
	case 'fsave':
		echo $users->save_froles();
	break;
	case 'ssave':
		echo $users->save_roles();
	break;
	case 'delete':
		echo $users->delete_roles();
	break;
	default:
		// echo $sysset->index();
		break;
