...
class SystemSettings extends DBConnection
{
    ...
    {
//	    calls the constructor of the parent class (DB connection). This ensures that the DB connection is established when an object of the 'SystemConnection' class is established.
        parent::__construct();
    }
...
//        execute the sql query by calling query() method on the database object ($this->conn).
        $qry = $this->conn->query($sql);
//        fetch_assoc() retrieves the next row from the query result. The loop continues until as long as there ane no more rows to fetch
        while ($row = $qry->fetch_assoc()) {
//            stores the values from the current row of the query result into the $_SESSION['system_info'] array.
//            It uses the value of the meta_field column as the key and the value of the meta_value column as the corresponding value in the array.
    ...
    function update_system_info()
    {
        ...
        $qry = $this->conn->query($sql);
        while ($row = $qry->fetch_assoc()) {
//          checks if a specific key, represented by $row['meta_field'], exists in the $_SESSION['system_info'] array. If the key exists, it is unset
//          (removed) from the array using the unset() function. This ensures that any existing value for the key is cleared before updating it with a new value.
